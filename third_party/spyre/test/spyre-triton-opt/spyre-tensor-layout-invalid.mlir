...
// phys_op[0] = 4 is not a valid CoordOp (0=identity, 1=floordiv, 2=mod, 3=splat)
tt.func @bad_op_code(%desc: !tt.tensordesc<64x64xf32>) {
  // expected-error @below {{phys_op[0] must be 0 (identity), 1 (floordiv), 2 (mod) or 3 (splat), got 4}}
  tt.spyre_tensor_layout %desc
    {phys_src = array<i64: 1, 0, 1>,
...
// nor an identity+splat re-stick
tt.func @duplicate_identity(%desc: !tt.tensordesc<64x64xf32>) {
  // expected-error @below {{logical dim 0 appears in 2 physical dims; a repeated logical dim is only valid as a stick split (exactly one floordiv entry and one mod entry) or a splat re-stick (exactly one identity entry and one splat entry), got 2 identity, 0 floordiv, 0 mod, 0 splat}}
  tt.spyre_tensor_layout %desc
...
tt.func @duplicate_floordiv(%desc: !tt.tensordesc<64x64xf32>) {
  // expected-error @below {{logical dim 1 appears in 2 physical dims; a repeated logical dim is only valid as a stick split (exactly one floordiv entry and one mod entry) or a splat re-stick (exactly one identity entry and one splat entry), got 0 identity, 2 floordiv, 0 mod, 0 splat}}
  tt.spyre_tensor_layout %desc
    {phys_src = array<i64: 1, 0, 1>,
...
tt.func @triple_split(%desc: !tt.tensordesc<64x64xf32>) {
  // expected-error @below {{logical dim 1 appears in 3 physical dims; a repeated logical dim is only valid as a stick split (exactly one floordiv entry and one mod entry) or a splat re-stick (exactly one identity entry and one splat entry), got 1 identity, 1 floordiv, 1 mod, 0 splat}}
  tt.spyre_tensor_layout %desc
    {phys_src = array<i64: 1, 1, 1>,
     phys_op = array<i64: 1, 0, 2>,
     phys_arg = array<i64: 64, 0, 64>} : !tt.tensordesc<64x64xf32>
...
tt.func @zero_floordiv_arg(%desc: !tt.tensordesc<64x64xf32>) {
  // expected-error @below {{phys_arg[0] must be > 0 for a floordiv/mod/splat dim, got 0}}
  tt.spyre_tensor_layout %desc
    {phys_src = array<i64: 1, 0, 1>,
     phys_op = array<i64: 1, 0, 2>,
...
tt.func @zero_mod_arg(%desc: !tt.tensordesc<64x64xf32>) {
  // expected-error @below {{phys_arg[2] must be > 0 for a floordiv/mod/splat dim, got 0}}
  tt.spyre_tensor_layout %desc
    {phys_src = array<i64: 1, 0, 1>,
     phys_op = array<i64: 1, 0, 2>,
