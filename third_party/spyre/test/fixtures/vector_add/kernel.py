...
Six ``@triton.jit`` functions exercising tensor descriptors at increasing
...
- :func:`add_kernel_2d_scalar_dim` — 2D, but `M` is a scalar read from
  memory rather than a kernel argument. KTIR-structural only for now.
"""
