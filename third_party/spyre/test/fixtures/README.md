...
| `factory` | `VariantFactory` | Optional. Supplies whatever varies with the swept `params` combination. Subclass `VariantFactory` (`test/conftest.py`) and override the hooks you need; each is called per combination with the combination as kwargs, and returns `None` to supply nothing. `signature()`, `reference()` and `inputs()` give that field's value; `derived_params()` gives `{name: value}` for params that follow from a swept one, such as a stick layout from a dtype. Producing something the variant also declares itself is an error. See the hook docstrings for the ordering rules. |
