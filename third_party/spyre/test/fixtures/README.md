...
| `factory` | `VariantFactory` | Optional. Supplies whatever varies with the swept `params` combination. Subclass `VariantFactory` (`test/conftest.py`) and override the hooks you need; each is called per combination with the combination as kwargs, and `None` means "supply nothing". `signature()`, `reference()` and `inputs()` return that *field's* value — declaring a hook and the literal field it produces on one variant is an error. `derived_params()` instead returns `{name: value}` for *params* that follow from the swept ones, so a layout need not be spelled out beside the dtype it must agree with; deriving a name that is also declared in `params` is an error. Derived params are resolved after the registry key is built, so they never appear in it, and before params are split into constexprs and runtime args, so a derived name can still be either. Return values verbatim, not `(label, value)` pairs — there is no key for a label to name. |
