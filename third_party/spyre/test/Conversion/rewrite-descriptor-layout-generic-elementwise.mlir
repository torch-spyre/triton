...
// Case 1 -- elementwise, every operand split identically.
//
// The rule degenerates correctly here: all three markers split logical dim 1 at
// 64, so no operand holds a split dim whole and no map carries arithmetic. The
// physical dim order is the marker's, (K/64, M, K%64), so the emitted identity
// is over (ks, m, kl) rather than the logical order grown on the right.

#id = affine_map<(d0, d1) -> (d0, d1)>
#s2 = affine_set<(d0, d1) : (d0 >= 0, -d0 + 63 >= 0, d1 >= 0, -d1 + 127 >= 0)>
module {
// CHECK-LABEL:   tt.func @ew(
...
// CHECK:           %[[VAL_5:.*]] = ktdp.construct_memory_view %[[VAL_4]], sizes: [2, 64, 64], strides: [4096, 64, 1] {coordinate_set = #[[$ATTR_2]], memory_space = #ktdp.memory_space<global>} : memref<2x64x64xf32>
...
// CHECK:           %[[VAL_10:.*]] = ktdp.construct_access_tile %[[VAL_5]]{{\[}}%[[VAL_7]], %[[VAL_3]], %[[VAL_9]]] {access_tile_order = #[[$ATTR_0]], access_tile_set = #[[$ATTR_2]]} : memref<2x64x64xf32> -> !ktdp.access_tile<2x64x64xindex>
...
// CHECK:           tt.return
// CHECK:         }
tt.func @ew(%a: !tt.ptr<f32>, %b: !tt.ptr<f32>, %c: !tt.ptr<f32>) {
