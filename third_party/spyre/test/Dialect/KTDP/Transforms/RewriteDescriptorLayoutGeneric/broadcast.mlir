...
// CHECK: #[[$ATTR_2:.+]] = affine_map<(d0, d1, d2) -> (d1, 0)>
// CHECK: #[[$ATTR_3:.+]] = affine_set<(d0, d1, d2) : (d0 >= 0, -d0 + 1 >= 0, d1 >= 0, -d1 + 23 >= 0, d2 >= 0, -d2 + 63 >= 0)>
// CHECK: #[[$ATTR_4:.+]] = affine_set<(d0, d1) : (d0 >= 0, -d0 + 23 >= 0, d1 >= 0, -d1 >= 0)>
...
// CHECK:           %[[VAL_5:.*]] = ktdp.construct_memory_view %[[VAL_4]], sizes: [2, 24, 64], strides: [1536, 64, 1] {coordinate_set = #[[$ATTR_3]], memory_space = #ktdp.memory_space<global>} : memref<2x24x64xf32>
...
// CHECK:           %[[VAL_10:.*]] = ktdp.construct_access_tile %[[VAL_5]]{{\[}}%[[VAL_7]], %[[VAL_3]], %[[VAL_9]]] {access_tile_order = #[[$ATTR_0]], access_tile_set = #[[$ATTR_3]]} : memref<2x24x64xf32> -> !ktdp.access_tile<2x24x64xindex>
...
// CHECK:           %[[VAL_13:.*]] = ktdp.construct_memory_view %[[VAL_12]], sizes: [24, 1], strides: [1, 1] {coordinate_set = #[[$ATTR_4]], memory_space = #ktdp.memory_space<global>} : memref<24x1xf32>
// CHECK:           %[[VAL_14:.*]] = ktdp.construct_access_tile %[[VAL_13]]{{\[}}%[[VAL_3]], %[[VAL_3]]] {access_tile_order = #[[$ATTR_1]], access_tile_set = #[[$ATTR_4]]} : memref<24x1xf32> -> !ktdp.access_tile<24x1xindex>
...
// CHECK:           %[[VAL_17:.*]] = ktdp.construct_memory_view %[[VAL_16]], sizes: [2, 24, 64], strides: [1536, 64, 1] {coordinate_set = #[[$ATTR_3]], memory_space = #ktdp.memory_space<global>} : memref<2x24x64xf32>
...
// CHECK:           %[[VAL_22:.*]] = ktdp.construct_access_tile %[[VAL_17]]{{\[}}%[[VAL_19]], %[[VAL_3]], %[[VAL_21]]] {access_tile_order = #[[$ATTR_0]], access_tile_set = #[[$ATTR_3]]} : memref<2x24x64xf32> -> !ktdp.access_tile<2x24x64xindex>
// CHECK:           %[[VAL_23:.*]] = tensor.empty() : tensor<2x24x64xf32>
// CHECK:           %[[VAL_24:.*]] = linalg.generic {indexing_maps = [#[[$ATTR_0]], #[[$ATTR_2]], #[[$ATTR_0]]], iterator_types = ["parallel", "parallel", "parallel"]} ins(%[[VAL_11]], %[[VAL_15]] : tensor<2x24x64xf32>, tensor<24x1xf32>) outs(%[[VAL_23]] : tensor<2x24x64xf32>) {
